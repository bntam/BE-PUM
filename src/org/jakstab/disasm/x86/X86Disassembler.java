...
public class X86Disassembler implements Disassembler, X86Opcodes {
	...
	public final Instruction decodeInstruction(){
		...
		if (InstructionDecoder.readByte(code, byteIndex) == 100) {
			if (InstructionDecoder.readByte(code, byteIndex + 1) == 103
				...
				return new X86Instruction("pushl", new X86MemoryOperand(DataType.INT32, new X86SegmentRegister(4, "%fs"), 0), 6, 1152);
					...
					&& InstructionDecoder.readByte(code, byteIndex + 4) == 0
					&& InstructionDecoder.readByte(code, byteIndex + 5) == 0)
				return new X86MoveInstruction("movl", new X86MemoryOperand(DataType.INT32, new X86SegmentRegister(4, "%fs"), 0), new X86Register(4, "%esp"), 6, 1152);
			else if (InstructionDecoder.readByte(code, byteIndex + 1) == 103
					&& InstructionDecoder.readByte(code, byteIndex + 2) == 143
					&& InstructionDecoder.readByte(code, byteIndex + 3) == 6
					&& InstructionDecoder.readByte(code, byteIndex + 4) == 0
					&& InstructionDecoder.readByte(code, byteIndex + 5) == 0)
				return new X86Instruction("popl", new X86MemoryOperand(DataType.INT32, new X86SegmentRegister(4, "%fs"), 0), 6, 1152);
		}

		if (InstructionDecoder.readByte(code, byteIndex) == 192 && InstructionDecoder.readByte(code, byteIndex + 1) == 240
				&& InstructionDecoder.readByte(code, byteIndex + 2) == 3)
			InstructionDecoder.setByte(code, byteIndex + 1, 224);

		if (InstructionDecoder.readByte(code, byteIndex) == 102 && InstructionDecoder.readByte(code, byteIndex + 1) == 211
				&& InstructionDecoder.readByte(code, byteIndex + 2) == 246)
			InstructionDecoder.setByte(code, byteIndex + 2, 230);

		if (InstructionDecoder.readByte(code, byteIndex) == 211 && InstructionDecoder.readByte(code, byteIndex + 1) == 241)
			InstructionDecoder.setByte(code, byteIndex + 1, 225);

		if (InstructionDecoder.readByte(code, byteIndex) == 102 && InstructionDecoder.readByte(code, byteIndex + 1) == 152)
			return new X86Instruction("cbws", 2, 512);
		if (InstructionDecoder.readByte(code, byteIndex) == 152)
		...
		try {
			...
			if (instrDecoder == null) {
				logger.error("Cannot find decoder for opcode "
			...
			if (instr == null) {
				logger.error("Decoder " + instrDecoder.getClass().toString()
						+ " for opcode " + Long.toHexString(opcode)
	...
	public final Instruction decodeInstruction(long index) {
		...
		/*if (InstructionDecoder.readByte(code, byteIndex) == 209) {
			if (InstructionDecoder.readByte(code, byteIndex + 1) == 224) {
				System.out.println("Use Capstone for disassembly: ");
				byte[] opCode = new byte[15];
				for (int i=0; i<15; i++) {
					byte t = (byte)InstructionDecoder.readByte(code, byteIndex + i);;
					opCode[i] = t;
				}
				...
				//allInsn[0].;
				//instr = new X86ArithmeticInstruction(allInsn[0].mnemonic,  new X86Register(4, "%esp"), 1, null, 2, 1);
				//return instr;
			}
		}*/
	...
	private static final InstructionDecoder oneByteTable[] = {
			/* 00 */
			new ArithmeticDecoder("addb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.ADD),
			new ArithmeticDecoder("addS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.ADD),
			new ArithmeticDecoder("addb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.ADD),
			new ArithmeticDecoder("addS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.ADD),
			new ArithmeticDecoder("addb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.ADD),
			new ArithmeticDecoder("addS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.ADD),
			new InstructionDecoder("pushl", ADDR_REG, ES),
			new InstructionDecoder("popl", ADDR_REG, ES),
			/* 08 */
			new ArithmeticDecoder("orb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.OR),
			new ArithmeticDecoder("orS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.OR),
			new ArithmeticDecoder("orb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.OR),
			new ArithmeticDecoder("orS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.OR),
			new ArithmeticDecoder("orb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.OR),
			new ArithmeticDecoder("orS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.OR),
			new InstructionDecoder("pushl", ADDR_REG, CS),
			null, /* 0x0f extended opcode escape */
			/* 10 */
			new ArithmeticDecoder("adcb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.ADDC),
			new ArithmeticDecoder("adcS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.ADDC),
			new ArithmeticDecoder("adcb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.ADDC),
			new ArithmeticDecoder("adcS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.ADDC),
			new ArithmeticDecoder("adcb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.ADDC),
			new ArithmeticDecoder("adcS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.ADDC),
			new InstructionDecoder("pushl", ADDR_REG, SS),
			new InstructionDecoder("popl", ADDR_REG, SS),
			/* 18 */
			new ArithmeticDecoder("sbbb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.SUBC),
			new ArithmeticDecoder("sbbS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.SUBC),
			new ArithmeticDecoder("sbbb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.SUBC),
			new ArithmeticDecoder("sbbS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.SUBC),
			new ArithmeticDecoder("sbbb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.SUBC),
			new ArithmeticDecoder("sbbS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.SUBC),
			new InstructionDecoder("pushl", ADDR_REG, DS),
			new InstructionDecoder("popl", ADDR_REG, DS),
			/* 20 */
			new ArithmeticDecoder("andb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.AND),
			new ArithmeticDecoder("andS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.AND),
			new ArithmeticDecoder("andb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.AND),
			new ArithmeticDecoder("andS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.AND),
			new ArithmeticDecoder("andb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.AND),
			new ArithmeticDecoder("andS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.AND),
			null, /* SEG es prefix */
			new InstructionDecoder("daa"),
			/* 28 */
			new ArithmeticDecoder("subb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.SUB),
			new ArithmeticDecoder("subS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.SUB),
			new ArithmeticDecoder("subb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.SUB),
			new ArithmeticDecoder("subS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.SUB),
			new ArithmeticDecoder("subb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.SUB),
			new ArithmeticDecoder("subS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.SUB),
			null, /* SEG CS prefix */
			new InstructionDecoder("das"),
			/* 30 */
			new ArithmeticDecoder("xorb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.XOR),
			new ArithmeticDecoder("xorS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.XOR),
			new ArithmeticDecoder("xorb", ADDR_G, b_mode, ADDR_E, b_mode,
					Operation.XOR),
			new ArithmeticDecoder("xorS", ADDR_G, v_mode, ADDR_E, v_mode,
					Operation.XOR),
			new ArithmeticDecoder("xorb", ADDR_REG, AL, ADDR_I, b_mode,
					Operation.XOR),
			new ArithmeticDecoder("xorS", ADDR_REG, EAX, ADDR_I, v_mode,
					Operation.XOR),
			null, /* SEG SS prefix */
			...
			new ArithmeticDecoder("imulS", ADDR_G, v_mode, ADDR_E, v_mode,
					ADDR_I, v_mode, Operation.SMUL),
			...
			new InstructionDecoder("movsb", ADDR_ESDI, b_mode, ADDR_DSSI,
					b_mode),
			new InstructionDecoder("movsS", ADDR_ESDI, v_mode, ADDR_DSSI,
					v_mode),
			new InstructionDecoder("cmpsb", ADDR_ESDI, b_mode, ADDR_DSSI,
					b_mode),
			new InstructionDecoder("cmpsS", ADDR_ESDI, v_mode, ADDR_DSSI,
					v_mode),
			/* a8 */
			...
			new RetDecoder("lret", ADDR_I, w_mode), new RetDecoder("lret"),
			new InstructionDecoder("int3"),
			new InstructionDecoder("int", ADDR_I, b_mode),
			new InstructionDecoder("into"), new RetDecoder("iret"),
			/* d0 */
			new GRPDecoder(null, 5), new GRPDecoder(null, 6),
			new GRPDecoder(null, 7), new GRPDecoder(null, 8),
			new InstructionDecoder("aam", ADDR_I, b_mode),
			new InstructionDecoder("aad", ADDR_I, b_mode), null,
			new InstructionDecoder("xlat"),
			/* d8 */
			new FloatDecoder(), new FloatDecoder(), new FloatDecoder(),
			new FloatDecoder(), new FloatDecoder(), new FloatDecoder(),
			new FloatDecoder(), new FloatDecoder(),
			/* e0 */
			...
			new InstructionDecoder("hlt"), new InstructionDecoder("cmc"),
			new GRPDecoder(null, 9), new GRPDecoder(null, 10),
			/* f8 */
			new InstructionDecoder("clc"), new InstructionDecoder("stc"),
			new InstructionDecoder("cli"), new InstructionDecoder("sti"),
			new InstructionDecoder("cld"), new InstructionDecoder("std"),
	...
	private static final InstructionDecoder twoByteTable[] = {
			...
			new SSEInstructionDecoder("unpcklps", ADDR_V, ps_mode, ADDR_W,
					q_mode),
			new SSEInstructionDecoder("unpckhps", ADDR_V, ps_mode, ADDR_W,
					q_mode),
			...
			new SSEInstructionDecoder("cvtpi2ps", ADDR_V, ps_mode, ADDR_Q,
					q_mode),
			new SSEMoveDecoder("movntps", ADDR_W, ps_mode, ADDR_V, ps_mode),
			new SSEInstructionDecoder("cvttps2pi", ADDR_Q, q_mode, ADDR_W,
					ps_mode),
			new SSEInstructionDecoder("cvtps2pi", ADDR_Q, q_mode, ADDR_W,
					ps_mode),
			new SSEInstructionDecoder("ucomiss", ADDR_V, ss_mode, ADDR_W,
					ss_mode),
			new SSEInstructionDecoder("comiss", ADDR_V, ps_mode, ADDR_W,
					ps_mode),
			/* 30 */
			...
			new SSEInstructionDecoder("sqrtps", ADDR_V, ps_mode, ADDR_W,
					ps_mode),
			new SSEInstructionDecoder("rsqrtps", ADDR_V, ps_mode, ADDR_W,
					ps_mode),
			new SSEInstructionDecoder("rcpps", ADDR_V, ps_mode, ADDR_W, ps_mode),
			new SSEArithmeticDecoder("andps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.AND),
			new SSEArithmeticDecoder("andnps", ADDR_V, ps_mode, ADDR_W,
					ps_mode, Operation.AND),
			new SSEArithmeticDecoder("orps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.OR),
			new SSEArithmeticDecoder("xorps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.XOR),
			/* 58 */
			new SSEArithmeticDecoder("addps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("mulps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.SMUL),
			new SSEInstructionDecoder("cvtps2pd", ADDR_V, pd_mode, ADDR_W,
					ps_mode),
			new SSEInstructionDecoder("cvtdq2ps", ADDR_V, ps_mode, ADDR_W,
					dq_mode),
			new SSEArithmeticDecoder("subps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.SUB),
			new SSEInstructionDecoder("minps", ADDR_V, ps_mode, ADDR_W, ps_mode),
			new SSEArithmeticDecoder("divps", ADDR_V, ps_mode, ADDR_W, ps_mode,
					Operation.SDIV),
			new SSEInstructionDecoder("maxps", ADDR_V, ps_mode, ADDR_W, ps_mode),
			/* 60 */
			new SSEInstructionDecoder("punpcklbw", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("punpcklwd", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("punpckldq", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("packsswb", ADDR_P, q_mode, ADDR_Q,
					q_mode),
			...
			new SSEInstructionDecoder("packuswb", ADDR_P, q_mode, ADDR_Q,
					q_mode),
			/* 68 */
			new SSEInstructionDecoder("punpckhbw", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("punpckhwd", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("punpckhdq", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			new SSEInstructionDecoder("packssdw", ADDR_P, q_mode, ADDR_Q,
					d_mode),
			null,
			null,
			...
			/* 70 */
			new SSEInstructionDecoder("pshufw", ADDR_P, q_mode, ADDR_Q, q_mode,
					ADDR_I, b_mode),
			...
			new InstructionDecoder("shldS", ADDR_E, v_mode, ADDR_G, v_mode,
					ADDR_I, b_mode),
			new InstructionDecoder("shldS", ADDR_E, v_mode, ADDR_G, v_mode,
					ADDR_REG, CL),
			null,
			...
			new InstructionDecoder("shrdS", ADDR_E, v_mode, ADDR_G, v_mode,
					ADDR_I, b_mode),
			new InstructionDecoder("shrdS", ADDR_E, v_mode, ADDR_G, v_mode,
					ADDR_REG, CL),
			...
			/* c0 */
			new ArithmeticDecoder("xaddb", ADDR_E, b_mode, ADDR_G, b_mode,
					Operation.ADD),
			new ArithmeticDecoder("xaddS", ADDR_E, v_mode, ADDR_G, v_mode,
					Operation.ADD),
			new SSEInstructionDecoder("cmpps", ADDR_V, ps_mode, ADDR_W,
					ps_mode, ADDR_I, b_mode),
			new SSEMoveDecoder("movnti", ADDR_E, d_mode, ADDR_G, d_mode),
			new SSEInstructionDecoder("pinsrw", ADDR_P, q_mode, ADDR_E, d_mode,
					ADDR_I, b_mode),
			new SSEInstructionDecoder("pextrw", ADDR_G, d_mode, ADDR_P, q_mode,
					ADDR_I, b_mode),
			new SSEInstructionDecoder("shufps", ADDR_V, ps_mode, ADDR_W,
					ps_mode, ADDR_I, b_mode),
			...
			/* d0 */
			null,
			new SSEArithmeticDecoder("psrlw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("psrld", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("psrlq", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("paddq", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("pmullw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SMUL),
			null,
			new SSEMoveDecoder("pmovmskb", ADDR_G, d_mode, ADDR_P, q_mode),
			/* d8 */
			new SSEArithmeticDecoder("psubusb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubusw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEInstructionDecoder("pminub", ADDR_P, q_mode, ADDR_Q, q_mode),
			new SSEArithmeticDecoder("pand", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.AND),
			new SSEArithmeticDecoder("paddusb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddusw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEInstructionDecoder("pmaxub", ADDR_P, q_mode, ADDR_Q, q_mode),
			new SSEArithmeticDecoder("pandn", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.AND),
			/* e0 */
			...
			new SSEArithmeticDecoder("pmulhuw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.UMUL),
			new SSEArithmeticDecoder("pmulhw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SMUL),
			null,
			new SSEMoveDecoder("movntq", ADDR_W, q_mode, ADDR_V, q_mode),
			/* e8 */
			new SSEArithmeticDecoder("psubsb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubsw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEInstructionDecoder("pminsw", ADDR_P, q_mode, ADDR_Q, q_mode),
			new SSEArithmeticDecoder("por", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.OR),
			new SSEArithmeticDecoder("paddsb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddsw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEInstructionDecoder("pmaxsw", ADDR_P, q_mode, ADDR_Q, q_mode),
			new SSEArithmeticDecoder("pxor", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.XOR),
			/* f0 */
			null,
			new SSEArithmeticDecoder("psllw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("pslld", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("psllq", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("pmuludq", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.UMUL),
			new SSEArithmeticDecoder("pmaddwd", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("psadbw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEMoveDecoder("maskmoveq", ADDR_P, pi_mode, ADDR_Q, pi_mode),
			/* f8 */
			new SSEArithmeticDecoder("psubb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubd", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubq", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("paddb", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddw", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddd", ADDR_P, q_mode, ADDR_Q, q_mode,
					Operation.ADD), null };
	...
	private static final InstructionDecoder twoBytePrefixF2Table[] = {
			...
			null,
			null,
			new SSEInstructionDecoder("cvtsi2sd", ADDR_V, sd_mode, ADDR_E,
					d_mode),
			null,
			new SSEInstructionDecoder("cvttsd2si", ADDR_G, d_mode, ADDR_W,
					sd_mode),
			new SSEInstructionDecoder("cvtsd2si", ADDR_G, d_mode, ADDR_W,
					sd_mode),
			null,
			null,
			/* 30 */
			...
			/* 50 */
			null,
			new SSEInstructionDecoder("sqrtsd", ADDR_V, sd_mode, ADDR_W,
					sd_mode),
			null,
			null,
			null,
			...
			null,
			/* 58 */
			new SSEArithmeticDecoder("addsd", ADDR_V, sd_mode, ADDR_W, sd_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("mulsd", ADDR_V, sd_mode, ADDR_W, sd_mode,
					Operation.SMUL),
			new SSEInstructionDecoder("cvtsd2ss", ADDR_V, sd_mode, ADDR_W,
					sd_mode),
			null,
			new SSEArithmeticDecoder("subsd", ADDR_V, sd_mode, ADDR_W, sd_mode,
					Operation.SUB),
			new SSEInstructionDecoder("minsd", ADDR_V, sd_mode, ADDR_W, sd_mode),
			new SSEArithmeticDecoder("divsd", ADDR_V, sd_mode, ADDR_W, sd_mode,
					Operation.SDIV),
			...
			null,
			/* 70 */
			new SSEInstructionDecoder("pshuflw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, ADDR_I, b_mode),
			null,
			null,
			null,
			null,
			...
			null,
			null,
			new SSEInstructionDecoder("cmpsd", ADDR_V, sd_mode, ADDR_W,
					sd_mode, ADDR_I, b_mode),
			null,
			null,
			null,
			null,
			...
			null,
			null,
			new SSEInstructionDecoder("cvtpd2dq", ADDR_V, dq_mode, ADDR_W,
					pd_mode), null,
			/* e8 */
	...
	private static final InstructionDecoder twoBytePrefixF3Table[] = {
			...
			null,
			null,
			new SSEInstructionDecoder("cvtsi2ss", ADDR_V, ss_mode, ADDR_E,
					d_mode),
			null,
			new SSEInstructionDecoder("cvttss2si", ADDR_G, d_mode, ADDR_W,
					ss_mode),
			new SSEInstructionDecoder("cvtss2si", ADDR_G, d_mode, ADDR_W,
					ss_mode),
			null,
			null,
			/* 30 */
			...
			/* 50 */
			null,
			new SSEInstructionDecoder("sqrtss", ADDR_V, ss_mode, ADDR_W,
					ss_mode),
			new SSEInstructionDecoder("rsqrtss", ADDR_V, ss_mode, ADDR_W,
					ss_mode),
			...
			null,
			/* 58 */
			new SSEArithmeticDecoder("addss", ADDR_V, ss_mode, ADDR_W, ss_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("mulss", ADDR_V, ss_mode, ADDR_W, ss_mode,
					Operation.SMUL),
			new SSEInstructionDecoder("cvtss2sd", ADDR_V, ss_mode, ADDR_W,
					ss_mode),
			new SSEInstructionDecoder("cvttps2dq", ADDR_V, dq_mode, ADDR_W,
					ps_mode),
			new SSEArithmeticDecoder("subss", ADDR_V, ss_mode, ADDR_W, ss_mode,
					Operation.SUB),
			new SSEInstructionDecoder("minss", ADDR_V, ss_mode, ADDR_W, ss_mode),
			new SSEArithmeticDecoder("divss", ADDR_V, ss_mode, ADDR_W, ss_mode,
					Operation.SDIV),
			...
			/* 70 */
			new SSEInstructionDecoder("pshufhw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, ADDR_I, b_mode),
			null,
			null,
			null,
			...
			null,
			null,
			new SSEInstructionDecoder("cmpss", ADDR_V, ss_mode, ADDR_W,
					ss_mode, ADDR_I, b_mode),
			null,
			null,
			null,
			null,
			...
			null,
			null,
			new SSEInstructionDecoder("cvtdq2pd", ADDR_V, pd_mode, ADDR_W,
					dq_mode), null,
			/* e8 */
			null, null, null, null, null, null, null, null,
			/* f0 */
	...
	private static final InstructionDecoder twoBytePrefix66Table[] = {
			...
			new SSEInstructionDecoder("unpcklpd", ADDR_V, pd_mode, ADDR_W,
					q_mode),
			new SSEInstructionDecoder("unpckhpd", ADDR_V, pd_mode, ADDR_W,
					q_mode),
			...
			new SSEInstructionDecoder("cvtpi2pd", ADDR_V, pd_mode, ADDR_Q,
					dq_mode),
			new SSEMoveDecoder("movntpd", ADDR_W, pd_mode, ADDR_V, pd_mode),
			new SSEInstructionDecoder("cvttpd2pi", ADDR_Q, dq_mode, ADDR_W,
					pd_mode),
			new SSEInstructionDecoder("cvtpd2pi", ADDR_Q, dq_mode, ADDR_W,
					pd_mode),
			new SSEInstructionDecoder("ucomisd", ADDR_V, sd_mode, ADDR_W,
					sd_mode),
			new SSEInstructionDecoder("comisd", ADDR_V, sd_mode, ADDR_W,
					sd_mode),
			/* 30 */
			null,
			null,
			...
			new SSEInstructionDecoder("sqrtpd", ADDR_V, pd_mode, ADDR_W,
					pd_mode),
			null,
			null,
			new SSEArithmeticDecoder("andpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.AND),
			new SSEArithmeticDecoder("andnpd", ADDR_V, pd_mode, ADDR_W,
					pd_mode, Operation.AND),
			new SSEArithmeticDecoder("orpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.OR),
			new SSEArithmeticDecoder("xorpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.XOR),
			/* 58 */
			new SSEArithmeticDecoder("addpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("mulpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.SMUL),
			new SSEInstructionDecoder("cvtpd2ps", ADDR_V, ps_mode, ADDR_W,
					pd_mode),
			new SSEInstructionDecoder("cvtps2dq", ADDR_V, dq_mode, ADDR_W,
					ps_mode),
			new SSEArithmeticDecoder("subpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.SUB),
			new SSEInstructionDecoder("minpd", ADDR_V, pd_mode, ADDR_W, pd_mode),
			new SSEArithmeticDecoder("divpd", ADDR_V, pd_mode, ADDR_W, pd_mode,
					Operation.SDIV),
			new SSEInstructionDecoder("maxpd", ADDR_V, pd_mode, ADDR_W, pd_mode),
			/* 60 */
			new SSEInstructionDecoder("punpcklbw", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("punpcklwd", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("punpckldq", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("packsswb", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("pcmpgtb", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("pcmpgtw", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("pcmpgtd", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("packuswb", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			/* 68 */
			new SSEInstructionDecoder("punpckhbw", ADDR_P, dq_mode, ADDR_Q,
					dq_mode),
			new SSEInstructionDecoder("punpckhwd", ADDR_P, dq_mode, ADDR_Q,
					dq_mode),
			new SSEInstructionDecoder("punpckhdq", ADDR_P, dq_mode, ADDR_Q,
					dq_mode),
			new SSEInstructionDecoder("packssdw", ADDR_P, dq_mode, ADDR_Q,
					dq_mode),
			new SSEInstructionDecoder("punpcklqdq", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("punpckhqdq", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEMoveDecoder("movd", ADDR_V, dq_mode, ADDR_E, d_mode),
			new SSEMoveDecoder("movdqa", ADDR_V, dq_mode, ADDR_W, dq_mode),
			/* 70 */
			new SSEInstructionDecoder("pshufd", ADDR_V, dq_mode, ADDR_W,
					dq_mode, ADDR_I, b_mode),
			new GRPDecoder(null, 22),
			new GRPDecoder(null, 23),
			new GRPDecoder(null, 24),
			new SSEInstructionDecoder("pcmpeqb", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("pcmpeqw", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEInstructionDecoder("pcmpeqd", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			null,
			/* 78 */
			null,
			...
			null,
			null,
			new SSEInstructionDecoder("cmppd", ADDR_V, pd_mode, ADDR_W,
					pd_mode, ADDR_I, b_mode),
			null,
			new SSEInstructionDecoder("pinsrw", ADDR_V, dq_mode, ADDR_E,
					d_mode, ADDR_I, b_mode),
			new SSEInstructionDecoder("pextrw", ADDR_G, d_mode, ADDR_V,
					dq_mode, ADDR_I, b_mode),
			new SSEInstructionDecoder("shufpd", ADDR_V, pd_mode, ADDR_W,
					pd_mode, ADDR_I, b_mode),
			null,
			/* c8 */
			null,
			...
			/* d0 */
			null,
			new SSEArithmeticDecoder("psrlw", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("psrld", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("psrlq", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SRL),
			new SSEArithmeticDecoder("paddq", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("pmullw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.SMUL),
			new SSEMoveDecoder("movq", ADDR_W, q_mode, ADDR_V, q_mode),
			new SSEMoveDecoder("pmovmskb", ADDR_G, d_mode, ADDR_V, dq_mode),
			/* d8 */
			new SSEArithmeticDecoder("psubusb", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.SUB),
			new SSEArithmeticDecoder("psubusw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.SUB),
			new SSEInstructionDecoder("pminub", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEArithmeticDecoder("pand", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.AND),
			new SSEArithmeticDecoder("paddusb", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEArithmeticDecoder("paddusw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEInstructionDecoder("pmaxub", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEArithmeticDecoder("pandn", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.AND),
			/* e0 */
			...
			new SSEArithmeticDecoder("pmulhuw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.UMUL),
			new SSEArithmeticDecoder("pmulhw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.SMUL),
			new SSEInstructionDecoder("cvttpd2dq", ADDR_V, dq_mode, ADDR_W,
					pd_mode),
			...
			new SSEArithmeticDecoder("psubusw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.SUB),
			new SSEInstructionDecoder("pminsw", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEArithmeticDecoder("por", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.OR),
			new SSEArithmeticDecoder("paddsb", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEArithmeticDecoder("paddsw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEInstructionDecoder("pmaxsw", ADDR_V, dq_mode, ADDR_W,
					dq_mode),
			new SSEArithmeticDecoder("pxor", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.XOR),
			/* f0 */
			null,
			new SSEArithmeticDecoder("psllw", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("pslld", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("psllq", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SLL),
			new SSEArithmeticDecoder("pmuludq", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.UMUL),
			new SSEArithmeticDecoder("pmaddwd", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEArithmeticDecoder("psadbw", ADDR_V, dq_mode, ADDR_W,
					dq_mode, Operation.ADD),
			new SSEMoveDecoder("maskmovdqu", ADDR_V, dq_mode, ADDR_W, dq_mode),
			/* f8 */
			new SSEArithmeticDecoder("psubb", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubw", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubd", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("psubq", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.SUB),
			new SSEArithmeticDecoder("paddb", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddw", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.ADD),
			new SSEArithmeticDecoder("paddd", ADDR_V, dq_mode, ADDR_W, dq_mode,
					Operation.ADD), null };
