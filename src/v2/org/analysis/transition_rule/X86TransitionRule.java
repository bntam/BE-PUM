...
public class X86TransitionRule extends TransitionRule {
	...
	boolean checkZ3(Formulas formulas) {
		...
		try {
			...
			Program.getProgram().getLog()
					.info("Done script. ExitCode:" + exitCode);
		} catch (Throwable e) {
			System.out.flush();
			e.printStackTrace();
	...
	void generateNextInstruction(Instruction ins, BPPath path,
			List<BPPath> pathList, boolean cond) {
		// TODO Auto-generated method stub
		...
		if (ins instanceof X86ArithmeticInstruction) {
			AbsoluteAddress newLocation = new AbsoluteAddress(curState
					.getLocation().getValue() + ins.getSize());

			Instruction newIns;
			// FOR ARITHMETIC HERE
			if (curState.getEnvironement().getSystem().isInVirtualMemory() == true) {
				byte[] opcodes = this.getOpcodesArray(curState,
						newLocation.getValue());
				newIns = Program.getProgram().getInstruction(opcodes,
						curState.getEnvironement());
			} else
				newIns = Program.getProgram().getInstruction(newLocation,
						curState.getEnvironement());
				...
				if (dest.getClass().getSimpleName()
					...
					r = new LongValue(
							((X86PCRelativeAddress) dest)
									.getEffectiveValue(targetTemp.getValue()));
				} else if (dest.getClass().getSimpleName()
						.equals("X86Register")) {
					r = env.getRegister().getRegisterValue(dest.toString());
				} else if (dest.getClass().getSimpleName()
						.equals("X86MemoryOperand")) {
					r = env.getMemory().getMemoryValue((X86MemoryOperand) dest,
							(X86Instruction) ins);
				}

				if (r != null && r instanceof LongValue) {
					AbsoluteAddress r1 = new AbsoluteAddress(
					...
					if (api != null/*!api.equals("")*/) {
						...
						if (curState.getEnvironement().getSystem()
								.isInVirtualMemory() == true) {
							byte[] opcodes = this.getOpcodesArray(curState,
									nextAddr.getValue());
							nextInst = Program.getProgram().getInstruction(
									opcodes, curState.getEnvironement());
						} else
							nextInst = Program.getProgram().getInstruction(
									nextAddr, env);
	...
	public void getNewState(BPPath path, List<BPPath> pathList, boolean cond) {
		...
		if (ins instanceof X86ArithmeticInstruction)
			new X86ArithmeticInterpreter().execute(
					(X86ArithmeticInstruction) ins, path, pathList, this);
		else if (ins instanceof X86CallInstruction)
			new X86CallInterpreter().execute((X86CallInstruction) ins, path,
					pathList, this);
		else if (ins instanceof X86CondJmpInstruction)
			new X86ConditionalJumpInterpreter().execute(
					(X86CondJmpInstruction) ins, path, pathList, this);
		else if (ins instanceof X86JmpInstruction)
			new X86JumpInterpreter().execute((X86JmpInstruction) ins, path,
					pathList, this);
		else if (ins instanceof X86MoveInstruction)
			new X86MoveInterpreter().execute((X86MoveInstruction) ins, path,
					pathList, this);
		else if (ins instanceof X86RetInstruction)
			new X86ReturnInterpreter().execute((X86RetInstruction) ins, path,
					pathList, this);
		else if (ins instanceof X86Instruction)
			new X86InstructionInterpreter().execute((X86Instruction) ins, path,
					pathList, this);
		if (!setCFG) {
			BPVertex dest = new BPVertex(curState.getLocation(),
					curState.getInstruction());
	...
	public void getNewState(X86CondJmpInstruction inst, BPPath path,
			List<BPPath> pathList, boolean condition) {
		// TODO Auto-generated method stub
		new X86ConditionalJumpInterpreter().execute(inst, path, pathList,
				condition, this);
		if (path.getCurrentState().checkFeasiblePath())
			LoopAlgorithm.getInstance().checkLoop(path, this);
	}
	...
	void multiDestination(Value r1, long value, Instruction inst, BPPath path,
	...
	// Old implementation
	protected void multiDestination_Old(Map<String, Long> z3Value,
			X86JmpInstruction inst, BPPath path, List<BPPath> pathList) {
		// TODO Auto-generated method stub
		...
		for (Entry<String, Long> i : z3Value.entrySet()) {
			...
			Value l1 = new HybridBooleanValue(new SymbolValue(var),
					new LongValue(value), "==");
			if (l2 != null)
	...
	public BPState processSEH(BPState curState) {
		...
		// Set up Stack Configuration for SEH
		Value e = new LongValue(curState.getEnvironement().getSystem()
				.getSEHHandler().getStart().getAddrSEHRecord());
		curState.getEnvironement().getStack()
		...
		// Address Code
		curState.getEnvironement().getStack()
				.push(new LongValue(curState.getLocation().getValue()));

		curState.getEnvironement().getStack().push(new LongValue(0));
		curState.getEnvironement().getStack().push(new LongValue(0));
		curState.getEnvironement().getStack()
				.push(new LongValue((long) (Math.random() * Math.pow(10, 7))));
		Value esp = curState.getEnvironement().getRegister()
				.getRegisterValue("esp");
	...
	*/
	 ...
	 */
	/*
	BPState specialProcessSEH(BPState curState) {
		// TODO Auto-generated method stub
		// PHONG
		Environment env = curState.getEnvironement();
		env.getRegister().mov("ebp", "esp");
		env.getRegister().add("ebp", new LongValue(20));
		long ebp_addr = ((LongValue) env.getRegister().getRegisterValue("ebp"))
				.getValue();
		long exception_took_place = ((LongValue) env.getMemory()
				.getDoubleWordMemoryValue(ebp_addr)).getValue();

		AbsoluteAddress nextAddr = new AbsoluteAddress(exception_took_place);
		Instruction nextIns = Program.getProgram()
				.getInstruction(nextAddr, env);

		// Restore stack
		long seh_address = curState.getEnvironement().getSystem()
				.getSEHHandler().getStart().getSehHandler();

		// Stack pointer
		while (true) {
			long top_stack_address = ((LongValue) env.getRegister()
					.getRegisterValue("esp")).getValue();
			Value t = env.getMemory().getDoubleWordMemoryValue(
					top_stack_address);

			if (t instanceof LongValue) {
				long top_stack_value = ((LongValue) t).getValue();
				env.getStack().pop();
				if (seh_address == top_stack_value || env.getStack().isEmpty())
					break;
			}
	...
	*/
	...
	public BPState specialProcessSEH(BPState curState) {
		// Restore
		long context_record_ptr = ((LongValue)curState.getEnvironement().getStack().getValueStackFromIndex(0x8)).getValue();
	...
	private void writeZ3Input(String fileName, Formulas formulaList) {
		...
		try {
			...
			try {
				...
				Program.getProgram()
						.getLog()
						.info(":extrafuns (" + formulaList.getVariableZ3()
								+ ")");
				...
				if (formulaList.isEmpty()) {
					...
					writer.write(":formula" + formulaList.toStringPrefix()
							+ "\n");
					Program.getProgram().getLog()
							.info(":formula" + formulaList.toStringPrefix());
				}
				writer.write(")\n");
	...
	public void getNewState(X86CondJmpInstruction inst, BPPath path,
		...
		if (new X86ConditionalJumpInterpreter().execute(inst, p1, path, true,
				this)) {			
			if (new X86ConditionalJumpInterpreter().execute(inst, p2, path,
					false, this)) {
				...
				if (path.getLoopHandle().isCheck()
						// && path.getLoopHandle().isStop()
							...
							path.getLoopHandle().setStop(
									newP.getLoopHandle()
											.checkFormulas(inst.getName(), newP.getCurrentState()));
			...
			if (path.getLoopHandle().isCheck())
				path.getLoopHandle().setStop(
						path.getLoopHandle()
							.checkFormulas(inst.getName(), path.getCurrentState()));
	...
	public Value getValueOperand(Operand src, Environment env, Instruction ins) {
		...
		if (src == null) {
			s = new LongValue(1);
				...
				.equals("Immediate")) {
			long y = (long)Convert.convetUnsignedValue(((Immediate) src)
					.getNumber().intValue(), getBitCount(ins));
			s = new LongValue(y);
		} else if (src.getClass().getSimpleName()
				.equals("X86Register")
			...
			s = env.getRegister().getRegisterValue(
					src.toString());
		} else if (src.getClass().getSimpleName()
				.equals("X86MemoryOperand")) {
