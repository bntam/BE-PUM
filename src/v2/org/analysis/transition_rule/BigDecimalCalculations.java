...
public class BigDecimalCalculations {
     ...
     */
    public BigDecimal sin(BigDecimal x, int scale)
    {
        if (x.signum() == 0) {
	 ...
	 * @return the result value
	 */
	public  BigDecimal tan(BigDecimal x, int scale) {
		if (x.signum() == 0) {
			return BigDecimal.ZERO;
		}
	...
	/**
	 * Compute the tangent of x to a given scale
	 * 
	 * @param x
	 *      the value of x  
	 * @param scale
	 ...
	 * @return the result value
	 */
	public BigDecimal tan_fix(BigDecimal angle, int scale){		
		BigDecimal sin = sin(angle, scale);
		BigDecimal cos = cos(angle, scale);
	...
	public BigDecimal log2(BigDecimal number){				
		BigDecimal two = new BigDecimal(2);
		BigDecimal result = (ln(number, 20)).divide(ln(two, 20),MathContext.DECIMAL128);
		return  result;
	}
