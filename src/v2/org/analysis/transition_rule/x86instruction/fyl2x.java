...
public class fyl2x extends X86InstructionStub {
	...
	public BPState execute() {
		...
		if (st0 == null || st1 == null) {
			...
			// ST0 = -infinity ,ST0 < 0, (ST0 == 1 && ST1 == +-infiity)
			// ST0 = +infinity, ST1 = 0
			// ==>IE
			else if ((temp_st0 < 0) || (temp_st0 == 0 && temp_st1 == 0)
					|| (temp_st0 == 1 && Double.isInfinite(temp_st1))
					|| (Double.isInfinite(temp_st0) && temp_st1 == 0)){
				env.getFST().setIE(new BooleanValue(true));
				env.getFST().setC1(new BooleanValue(false));
				env.getFPUregister().setFPURegisterValue("st1", new DoubleValue(Double.NaN));						
			}
			// ST0 == 0, ST1 = -F => ZE = true, ST1 = +infinity
			else if (temp_st0 == 0 && temp_st1 < 0 && Double.isFinite(temp_st1)){
				env.getFPUregister().setFPURegisterValue("st1", new DoubleValue(Double.POSITIVE_INFINITY));		
				env.getFST().setZE(new BooleanValue(true));
			}
			// ST0 == 0, ST1 = +F => ZE = true, ST1 = -infinity
			else if (temp_st0 == 0 && temp_st1 > 0 && Double.isFinite(temp_st1)){
				env.getFPUregister().setFPURegisterValue("st1", new DoubleValue(Double.NEGATIVE_INFINITY));	
				env.getFST().setZE(new BooleanValue(true));
			}
			// ST0 < 1 && ST1 = -infinity 
			// ST0 > 1 && ST1 = +infinity
			// ST0 = +infinity && ST1 > 0 
			// =>ST1 = +infinity
			else if ((temp_st0 < 1 && Double.isInfinite(temp_st1) && temp_st1 < 0)
					|| (temp_st0 > 1 && Double.isInfinite(temp_st1) && temp_st1 > 0) 
					|| (Double.isInfinite(temp_st0) && temp_st0 > 0 && temp_st1 > 0)){
				env.getFPUregister().setFPURegisterValue("st1", new DoubleValue(Double.POSITIVE_INFINITY));		
			}
			// ST0 > 1 && ST1 = -infinity 
			// ST0 < 1 && ST1 = +infinity
			...
			// ST0 == 1 || ST1 == 0
			// => ST1 = 0
			else if ( (temp_st0 == 1) || (temp_st1 == 0) ){
				env.getFPUregister().setFPURegisterValue("st1", new DoubleValue(0.0));
			}
			// finity
			else {
				double result = temp_st1 * log2(temp_st0);
				...
				BigDecimal temp_0 = new BigDecimal(temp_st0);
