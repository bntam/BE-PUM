...
public class X86ArithmeticInterpreter {
	public BPState execute(X86ArithmeticInstruction ins,
						   BPPath path, List<BPPath> pathList, X86TransitionRule rule) {
		...
		switch (ins.getOperation()) {
				...
				if (ins.getName().startsWith("div")) {
					...
					switch (opSize) {
						...
						case 16:
							...
							if (ax != null && ax instanceof LongValue && src != null && src instanceof LongValue
									&& dx != null && dx instanceof LongValue) {
								long vAX = ((LongValue)ax).getValue();
								long vSRC = ((LongValue)src).getValue();
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (src.getClass().getSimpleName().equals("X86Register")
							...
							s = env.getRegister().getRegisterValue(src.toString());
								...
								.equals("Immediate")) {
							s = new LongValue((long)Convert.convetUnsignedValue(
									((Immediate) src).getNumber().intValue(),
									rule.getBitCount(ins)));
						} else if (src.getClass().getSimpleName()
								.equals("X86MemoryOperand")) {
							s = env.getMemory().getMemoryValue(
						...
						if (d != null && s != null) {
							...
							env.getFlag().changeFlagWithUDIV(d, s, env,
									rule.getBitCount(ins));
				...
				if (ins.getName().startsWith("add")) {
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (s != null) {
							...
							env.getFlag().changeFlagWithADD(d, s, env,
									rule.getBitCount(ins));
						}
						...
						if (s != null) {
							...
							env.getMemory().addMemoryValue((X86MemoryOperand) dest,
									s, ins);
							env.getFlag().changeFlagWithADD(d, s, env,
									rule.getBitCount(ins));
						}
				...
				if (ins.getName().startsWith("adc")) {
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (s != null) {
							...
							if (cf instanceof BooleanValue) {
								if (((BooleanValue) cf).getValue()) {
									env.getRegister().add(dest.toString(), s);
									env.getRegister().add(dest.toString(),
											new LongValue(1));
									env.getFlag().changeFlagWithADC(d, s, 1, env,
											rule.getBitCount(ins));

								} else {
									env.getRegister().add(dest.toString(), s);
									env.getFlag().changeFlagWithADC(d, s, 0, env,
											rule.getBitCount(ins));
								}
						...
						if (s != null) {
							...
							if (cf instanceof BooleanValue) {
								if (((BooleanValue) cf).getValue()) {
									env.getMemory().addMemoryValue(
											(X86MemoryOperand) dest, s, ins);
									env.getMemory().addMemoryValue(
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
							...
							|| dest.getClass().getSimpleName()
							.equals("X86SegmentRegister")) {
						Value d = env.getRegister().getRegisterValue(
								dest.toString());
						env.getRegister().add(dest.toString(), new LongValue(1));
						env.getFlag().changeFlagWithINC(d, env, rule.getBitCount(ins));
					} else if (dest.getClass().getSimpleName()
						...
						Value d = env.getMemory().getMemoryValue(
				...
				if (ins.getName().startsWith("and")) {
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (s != null) {
							...
							env.getFlag().changeFlagWithAND(d, s, env,
									rule.getBitCount(ins));
						}
					} else if (dest.getClass().getSimpleName()
							.equals("X86MemoryOperand")) {
						...
						if (s != null) {
							...
							// env.getFlag().changeFlagWithADD(false, d, s,
							// env,rule.getBitCount(ins));
							env.getMemory().andMemoryValue((X86MemoryOperand) dest,
									s, ins);
							env.getFlag().changeFlagWithAND(d, s, env,
									rule.getBitCount(ins));
						}
					...
					if (dest.toString().equals(src.toString())) {
						if (dest.getClass().getSimpleName().equals("X86Register")
							...
							env.getRegister().setRegisterValue(dest.toString(),
									new LongValue(0));
							env.getFlag().changeFlagWithXOR(new LongValue(1),
									new LongValue(1), env, rule.getBitCount(ins));
						} else if (dest.getClass().getSimpleName()
								.equals("X86MemoryOperand")) {
						...
						if (s != null) {
							...
							env.getFlag().changeFlagWithXOR(d, s, env,
									rule.getBitCount(ins));
						}
					} else if (dest.getClass().getSimpleName()
							.equals("X86MemoryOperand")) {
						...
						if (s != null) {
							...
							// env.getFlag().changeFlagWithADD(false, d, s,
							// env,rule.getBitCount(ins));
							env.getMemory().xorMemoryValue((X86MemoryOperand) dest,
									s, ins);
							env.getFlag().changeFlagWithXOR(d, s, env,
									rule.getBitCount(ins));
						}
			...
			/* Statement supported: sub, dec */
				if (ins.getName().startsWith("sub")) {
					...
					if (dest.toString().equals(src.toString())) {
						...
						if (s != null) {
							env.getMemory().subMemoryValue((X86MemoryOperand) dest,
									s, ins);
							env.getFlag().changeFlagWithSUB(d, s, env,
									rule.getBitCount(ins));
						}
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						env.getFlag()
								.changeFlagWithDEC(
										env.getRegister().getRegisterValue(
												dest.toString()), env,
										rule.getBitCount(ins));
						...
						env.getFlag().changeFlagWithDEC(
								env.getMemory().getMemoryValue(
										(X86MemoryOperand) dest, ins), env,
								rule.getBitCount(ins));
						env.getMemory().subMemoryValue((X86MemoryOperand) dest,
								new LongValue(1), ins);
					}
				...
				if (ins.getName().startsWith("or")) {
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (s != null) {
							...
							env.getFlag().changeFlagWithOR(d, s, env,
									rule.getBitCount(ins));
						}
					} else if (dest.getClass().getSimpleName()
							.equals("X86MemoryOperand")) {
						...
						if (s != null) {
							...
							// env.getFlag().changeFlagWithADD(false, d, s,
							// env,rule.getBitCount(ins));
							env.getMemory().orMemoryValue((X86MemoryOperand) dest,
									s, ins);
							env.getFlag().changeFlagWithOR(d, s, env,
									rule.getBitCount(ins));
						}
			...
			case SRL:
				if (ins.getName().startsWith("shr")) {
					...
					if (d != null && s != null && d instanceof LongValue && s instanceof LongValue) {
						long t = calculateShiftOperator(((LongValue)d).getValue(), ((LongValue)s).getValue(), env, rule, ins);
			...
			case RR:
				if (ins.getName().startsWith("ror")) {
					...
					if (d != null && s != null && d instanceof LongValue && s instanceof LongValue) {
						long t = calculateRotateOperator(((LongValue) d).getValue(), ((LongValue) s).getValue(), env, rule, ins);
						rule.setValueOperand(dest, new LongValue(t), env, ins);
						break;
					}
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (src == null) {
							...
							long y = (long)Convert.convetUnsignedValue(((Immediate) src)
									.getNumber().intValue(), rule.getBitCount(ins));
				...
				if (ins.getName().startsWith("rol")) {
					...
					if (dest.getClass().getSimpleName().equals("X86Register")
						...
						if (src == null) {
							...
							// sv.sub(dest.toString(), src.toString());
							s = env.getRegister().getRegisterValue(src.toString());
							env.getFlag().changeFlagWithRL(d, s, env,
									rule.getBitCount(ins));
					...
					switch (ins.getOperandCount()) {
						case 2:
							if (ins.getOperand1().toString().startsWith("%ax") || ins.getOperand1().toString().startsWith("ax") ||
									ins.getOperand1().toString().startsWith("%al") || ins.getOperand1().toString().startsWith("al")
									|| ins.getOperand1().toString().startsWith("%eax") || ins.getOperand1().toString().startsWith("eax")) {
								if (opSize == 8) {
									...
									if (ax != null && ax instanceof LongValue && source != null && source instanceof LongValue) {
										long temp = BitVector.signedMul(((LongValue) ax).getValue(), ((LongValue) source).getValue(), opSize);
										...
										//if (BitVector.signExtend(axVal, 32) == (axVal + dxVal * Math.pow(2, 16))) {
									...
									if (eax != null && eax instanceof LongValue && source != null && source instanceof LongValue) {
										long temp = BitVector.signedMul(((LongValue) eax).getValue(), ((LongValue) source).getValue(), opSize);
	...
	private long calculateShiftOperator(long dest, long count, Environment env, X86TransitionRule rule, X86ArithmeticInstruction inst) {
		if (count == 0) {
			//All flag unchanged
			return dest;
		}
