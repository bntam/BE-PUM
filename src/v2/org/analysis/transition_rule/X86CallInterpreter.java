...
public class X86CallInterpreter {
	public BPState execute(X86CallInstruction inst, BPPath path,
		...
		if (r != null && r instanceof LongValue) {
			...
			if (z3Value != null) {
				...
				if (!(r instanceof LongValue)) {
					Program.getProgram().getLog().debugString("Fail to resolve indirect jump at " + curState.getLocation());
					curState.setInstruction(null);
					curState.setLocation(null);
					return curState;
				}
				
				AbsoluteAddress r2 = new AbsoluteAddress(
						((LongValue) r).getValue());
				Program.getProgram()
						.getBPCFG()
						.getVertex(curState.getLocation(),
				...
				if (!api.equals("")) {
					rule.getAPIHandle().executeAPI(
							...
							api, inst, path, pathList);
					rule.setCFG(true);
				} else {
					if (r instanceof LongValue) {
						env.getStack().push(
								new LongValue(returnAddress.getValue()));
						...
						Instruction i = Program.getProgram().getInstruction(a,
								env);
