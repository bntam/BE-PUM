...
public class LoopAlgorithm {
	...
	public boolean checkLoop(BPPath path, X86TransitionRule rule) {
		...
		if (trace.contain(curState.getLocation().getValue())) {
			...
			if (entry != null
					&& curState.getLocation().getValue() == entry.getAddress()
							.getValue()
					&& curState.getInstruction().compareInstruction(
			...
			if (loopHead == null) {
				temp.setLoopHead(new BPVertex(curState.getLocation(), curState
						.getInstruction()));
				temp.setNumLoop(1);
				temp.setCheck(true);
				temp.setStop(false);
				...
				if (loop_num > MAX_LOOP
						|| loop_num > exceptionCase(Program.getProgram()
								.getFileName())) {
					//BPCFG cfg = Program.getProgram().getBPCFG();
					
					System.out.println("Loop node reach Maximum Loops counts "
							+ MAX_LOOP + " at " + curState.getLocation());
					path.setStop(true);
					return true;
				}

				if (loopHead.getAddress().getValue() == curState.getLocation()
						.getValue()
						&& loopHead.getInstruction().compareInstruction(
					...
					if (temp.getLoopTrace().size() == 1) {
						if (curState.getInstruction().getName().startsWith("call")
								|| curState.getInstruction().getName()
										.startsWith("jmp")) {
							if (curState.getEnvironement().getSystem().getSEHHandler()
									.isSet()) {
								System.out
										.println("Loop Stack Overflow - Exception: SEH Occur");
								...
								System.out
										.println("Loop Stack Overflow - Exception: Stop path");
								path.setStop(true);
								return true;
							}
						}
					...
					if (temp.isStop()) {
						System.out
							.println("Loop may run forever: Stop path after " + path.getLoopHandle().getNumLoop() + " times");
						System.out
						.println("Loop Path: " + path.getLoopHandle());
						path.setStop(true);
						return true;
					}
				}				
	...
	private int exceptionCase(String fileName) {
		// TODO Auto-generated method stub
		if (fileName != null
				&& (fileName.contains("count6.exe")
						|| fileName.contains("count7.exe")
						|| fileName.contains("count8.exe")
						|| fileName.contains("count9.exe")
						|| fileName.contains("smc.exe")
						|| fileName.contains("ex.exe") || fileName
