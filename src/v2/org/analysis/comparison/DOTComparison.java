...
public class DOTComparison {
	...
	private boolean compare(Vertex s, Vertex d) {
		...
		if (s.isAPI()) {
			...
			return compareStringInst(s.getInstructionString(),
					d.getInstructionString());
		}
	...
	private boolean compareInstOperands(String i1, String i2) {
		...
		if (d[0].contains("rep")
				|| (d[0].contains("shl") && s.length != d.length)) {
		...
		} else if (d[0].equals("jmp") && d[1].contains("@")
				&& s[1].contains("ds:imp")) {
			//System.out.println("Consider Equal:" + s[1] + " vs " + d[1]);
		...
		if (s.length == 2) {
			...
			return compareOperand(s[1], d[1]) && compareOperand(s[2], d[3])
					&& compareOperand(s[3], d[2]);
		}
	...
	private boolean compareOperand(String s, String d) {
		...
		if (s1.contains("+") || s1.contains("-") || s1.contains("*")
				|| s1.contains("/") || s1.startsWith("a")) {
			//System.out.println("Consider Equal:" + s1 + " vs " + d1);
		} else if (!s1.equals(d1) && !s2.equals(d2) && 
				checkSpecialCase(s1) && checkSpecialCase(d1))
			return false;
	...
	private String contain(CFG d, Vertex vertex) {
		...
		if (vertex.isAPI()) {
			for (Vertex t : verteces) {
				...
				String sx = "ds:__imp_"
						+ vertex.getInstructionString().toLowerCase();
				// return
				// d.getInstructionString().toLowerCase().contains("ds:__imp_" +
				// s.getInstructionString().toLowerCase());
				if (dx.contains(sx))
			...
			for (Vertex v : verteces) {
				if (compareStringAddr(v.getAddress(), vertex.getAddress())) {
					if (compareStringInst(v.getInstructionString(),
							vertex.getInstructionString()))
						return "";

					return "Vertex Change: " + vertex.getAddress() + ": "
							+ vertex.getInstructionString() + " vs "
	...
	private String contain(Edge edge1, CFG d) {
		...
		for (Edge edge2 : edges) {
			if (compare(edge2.getDest(), edge1.getDest())
					&& compare(edge2.getSource(), edge1.getSource()))
				return "";
	...
	public void exportComparison(String fileName) {
		...
		fp.appendFile("===============================================================================");
		System.out
				.println("Comparison between BE-PUM and IDA Pro on DOT file: "
						+ src + " vs " + dest + ":");
		fp.appendFile("Comparison between BE-PUM and IDA Pro on DOT file: "
				+ src + " vs " + dest + ":");
		System.out
				.println("===============================================================================");
		fp.appendFile("---------------------------------------------------------------");
	...
	private void generateCFGForIDA(String fileName) {
		try {
			Process p = Runtime.getRuntime().exec(
					"cmd /c start /wait ExportCFG.bat" + " " + fileName);
			Program.getProgram().getLog()
					.info("Running script ExportCFG.bat...");
	...
	private String normalizeMnemonic(String inst, String fullInst) {
		...
		if (inst.endsWith("s") || inst.endsWith("w") || inst.endsWith("b")
				|| inst.endsWith("l"))
	...
	private boolean specialCase(String s, String d) {
		// TODO Auto-generated method stub
		return (s.equals("jne") || s.equals("jnz"))
				&& (d.equals("jne") || d.equals("jnz"))
				|| (s.equals("je") || s.equals("jz"))
				&& (d.equals("je") || d.equals("jz"))
				|| (s.equals("jae") || s.equals("jnb"))
				&& (d.equals("jae") || d.equals("jnb"))
				|| (s.equals("repe") || s.equals("repz"))
				&& (d.equals("repe") || d.equals("repz"))
				|| (s.contains("movzx") || s.contains("movzwl"))
				&& (d.contains("movzwl") || d.equals("movzx"))
				|| (s.contains("movsx") || s.contains("movsbl"))
				&& (d.contains("movsbl") || d.equals("movsx"));
	}
