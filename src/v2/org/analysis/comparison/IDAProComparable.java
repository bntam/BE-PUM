...
public class IDAProComparable extends Comparison {
		...
		public boolean contain(Vertex t) {
			for (Vertex temp : vertices)
				if (temp.getAddress().getValue() == t.getAddress().getValue()
						&& temp.getInstructionString().equals(
								t.getInstructionString()))
					return true;
			return false;
		}
	...
	private String compare(BPVertex p, CFG cfg) {
		...
		for (Vertex t : verteces) {
			if (p != null && p.getAddress() != null
					&& t.getAddress().getValue() == p.getAddress().getValue()) {
				if (compare(t.getInstructionString(), p.getInstruction()))
					return "";
				else
					return p.getAddress() + ": " + t.getInstructionString() + "vs "
							+ getInstString(p.getInstruction());
			}
		}
	...
	public void compare(String fileName) {
		...
		System.out
				.println("===============================================================================");
		System.out.println("Comparison between IDA Pro and BE-PUM on "
				+ fileName + ":");
		System.out
				.println("===============================================================================");
	...
	private Vertex getVertex(String line) {
		...
		if (line.startsWith(".text:") || line.startsWith("CODE:")) {
			...
			if (v[1].startsWith(";") || v[1].equals("db") || v[1].equals("dd")
					|| v[1].startsWith("loc_") || v[1].startsWith(".")
					|| v[1].startsWith("_text") || v[1].equals("assume")
					|| v[1].equals("public") || v[1].startsWith("start")
					|| v[1].startsWith("unk") || v[1].startsWith("_"))
				return null;
			...
			AbsoluteAddress address = new AbsoluteAddress(Integer.parseInt(
					addr, 16));
			String property = "";
	...
	private void getLTS(String fileName) {
		try {
			Process p = Runtime.getRuntime().exec(
					"cmd /c start /wait ExportLST.bat" + " " + fileName);
			Program.getProgram().getLog()
					.info("Running script ExportLTS.bat...");
