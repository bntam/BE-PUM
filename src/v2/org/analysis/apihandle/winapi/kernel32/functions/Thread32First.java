...
public class Thread32First extends Kernel32API {
	...
	public boolean execute(AbsoluteAddress address, String funcName, BPState curState, Instruction inst) {
		...
		if (x1 instanceof LongValue && x2 instanceof LongValue) {
			...
			long index = t2;
			lpte.dwSize = new DWORD(((LongValue) memory.getDoubleWordMemoryValue(new X86MemoryOperand(DataType.INT32,
					index))).getValue());
			lpte.cntUsage = new DWORD(((LongValue) memory.getDoubleWordMemoryValue(new X86MemoryOperand(DataType.INT32,
					index += 4))).getValue());
			index += 4;
			lpte.th32ThreadID = new DWORD(((LongValue) memory.getDoubleWordMemoryValue(new X86MemoryOperand(
			...
			lpte.tpBasePri = new LONG(((LongValue) memory.getDoubleWordMemoryValue(new X86MemoryOperand(DataType.INT32,
					index += 4))).getValue());
			index += 4;
			lpte.tpDeltaPri = new LONG(((LongValue) memory.getDoubleWordMemoryValue(new X86MemoryOperand(
					DataType.INT32, index += 4))).getValue());
			index += 4;
